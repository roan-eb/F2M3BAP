...
// your data
String studentName = "Roan en mijn studenten nummer is 32800"; // jouw student nummer + naam
...
void handleRoot() {server.send(200, "text/html", "<h3>Groetjesvan DuurzaamHuis! mijnnaam is: "
  +  studentName+  "<h3> <form action=\"/led1\" method=\"GET\"><input type=\"submit\" value=\"Toggle Led1\"></form> "
 ...
 */
  ...
  server.on("/easter_egg", [](){
        server.send(200, "text/html", "<h1>Easter egg</h1>");
    });
  ...
  server.on("/led1", [](){
   ...
   server.on("/led2", [](){
    ...
    server.on("/led3", [](){
       if(!led2Status && !digitalRead(Led3)){
